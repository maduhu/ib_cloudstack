...
(function(cloudStack, $) {
    var pageElems = cloudStack.uiCustom.projectsTabs = {
	    ...
	    var $select = $('<select>').addClass('domain-select').attr({
		name: 'project-domain',
		id: 'project-domain'
	    });
		...
		success: function(data) {
                    domainObjs = data.listdomainsresponse.domain;
                }
	    });


            var $projectDesc = $('<div>').addClass('field desc')
                .append($('<label>').attr('for', 'project-display-text').html(_l('label.display.text')))
            ...
            $projectDomain = $('<div>').addClass('field domain')
                .append($('<label>').attr('for', 'domain-name').html(_l('label.domain')));

	       
	    for(var i = 0; i < domainObjs.length; i++){
		$options += '<option value="'+domainObjs[i].id+'">'+domainObjs[i].name+'</option>';
	    }

	    $select.append($options);
	    $projectDomain.append($select);
            ...
            $form.submit(function() {
				...
				$confirm.append($projectDomain).find('select').replaceWith(
				    $('<span>').addClass('value').html(_s(
